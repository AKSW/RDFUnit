    ...
    public void sourceGenerationExecuted(final Source source, final TestGenerationType generationType, final long testsCreated) {
        UI.getCurrent().access(new Runnable() {
            ...
            public void run() {
                ...
                String column = (generationType == TestGenerationType.AutoGenerated ? "Automatic" : "Manual");
                Property<Link> statusProperty = item.getItemProperty(column);
                String fileName = "";
                if (generationType == TestGenerationType.AutoGenerated)
                    fileName = CacheUtils.getSourceAutoTestFile(DatabuggerUISession.getBaseDir()+"tests/", source);
                else
                    fileName = CacheUtils.getSourceManualTestFile(DatabuggerUISession.getBaseDir() + "tests/", source);
