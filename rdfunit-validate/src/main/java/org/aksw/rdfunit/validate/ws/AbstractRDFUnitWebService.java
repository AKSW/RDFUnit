...
public abstract class AbstractRDFUnitWebService extends HttpServlet {
   ...
   */
  private static void writeResults(final RDFUnitConfiguration configuration,
      final TestExecution testExecution, HttpServletResponse httpServletResponse)
      throws RdfWriterException, IOException {
    ...
    RdfWriter rdfWriter = RdfResultsWriterFactory
        .createWriterFromFormat(httpServletResponse.getOutputStream(), serializationFormat,
            testExecution);
  ...
  /**
   * Has all the workflow logic for getting the parameters, performing a validation and writing the
   * output
   */
  private void handleRequestAndRespond(HttpServletRequest httpServletRequest,
      HttpServletResponse httpServletResponse) throws IOException {
   ...
   */
  protected abstract RDFUnitConfiguration getConfiguration(HttpServletRequest httpServletRequest)
   ...
   * @param testSource The dataset to be tested, this is generated automatically from the
   * RDFUnitConfiguration object
   * @return A TestSuite (list of test cases) for running against the tested Source
   */
  protected abstract TestSuite getTestSuite(final RDFUnitConfiguration configuration,
      final TestSource testSource);

  /**
   * Executes the validation of a Sourse dataset against a TestSuite based on a
   ...
   */
  protected abstract TestExecution validate(final RDFUnitConfiguration configuration,
      final TestSource testSource, final TestSuite testSuite) throws TestCaseExecutionException;
   ...
   */
  protected abstract void printHelpMessage(HttpServletResponse httpServletResponse)
      throws IOException;
   ...
   * @throws java.io.IOException if any.
   */
  protected void printMessage(HttpServletResponse httpServletResponse, String message)
      throws IOException {
