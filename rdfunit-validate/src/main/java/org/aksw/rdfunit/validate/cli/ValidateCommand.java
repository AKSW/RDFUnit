...
public class ValidateCommand implements Callable {
  ...
  static class DatasetOptions {
    ...
    @Option(names = { "-u", "--uri" }, description = "The URI to use for dereferencing a dump if not the same with the dataset URI (-d).")
  ...
  @Option(names = {"-o", "--output-format"}, description = "The output format of the validation results. One of `html` (default), `turtle`, `n3`, `ntriples`, `json-ld`, `rdf-json`, `rdfxml`, `rdfxml-abbrev`, `junitxml`.", defaultValue = "html")
  String outputFormat = "";

  @Option(names = {"-p", "--enriched-prefix"}, description = "The prefix of this dataset used for caching the schema enrichment.")
  String enrichedPrefix = "";

  @Option(names = {"-C", "--no-test-cache"}, description = "Do not load cached test cases, regenerate them (cached test cases are loaded by default).")
  boolean noTestCache = false;

  @Option(names = {"-M", "--no-manual-tests"}, description = "Do not load any manually defined test cases (manual test cases are loaded by default).")
  boolean noManualTests = false;

  @Option(names = {"-A", "--no-auto-tests"}, description = "Do not load any schema / automatically generated test cases (automatically generated test cases are loaded by default).")
  boolean noAutoTests = false;

  @Option(names = {"-r", "--result-level"}, description = "Specify the result level for the error reporting. One of `status`, `aggregate` (default), `rlog`, `extended.", defaultValue = "aggregate")
  String resultLevel = "";

  @Option(names = {"-l", "--logging-level"}, description = "NOT SUPPORTED at the moment! will filter test cases based on logging level (notice, warn, error, etc).")
  String loggingLevel = "";

  @Option(names = {"-T", "--query-cache-ttl"}, description = "The SPARQL endpoint cache Time-To-Live (TTL) (in minutes) or '0' to disable it.", defaultValue = "1440")
  int queryCacheTTL;

  @Option(names = {"-D", "--query-delay"}, description = "The delay between consecutive queries against an endpoint (in seconds) or '0' to disable delay.", defaultValue = "0")
  long queryDelay;

  @Option(names = {"-L", "--query-limit"}, description = "The maximum results from a SPARQL test query or '0' to disable limits.", defaultValue = "0")
  int queryLimit;

  @Option(names = {"-P", "--query-pagination"}, description = "The pagination page size for retrieving big results or '0' to disable it.", defaultValue = "0")
  long queryPagination;

  @Option(names = {"-c", "--test-coverage"}, description = "Calculate test-coverage scores.")
  boolean testCoverage = false;

  @Option(names = {"-f", "--data-folder"}, description = "The location of the data folder (defaults to './data/' or '~/.rdfunit/). This is where the results and the caches are stored. "
  ...
  private void setDumpOrSparqlEndpoint(RDFUnitConfiguration configuration) {
    ...
    if (datasetOptions != null && datasetOptions.customDereferenceURI != null && !datasetOptions.customDereferenceURI.isEmpty()) {
  ...
  private void setTestAutogetCacheManual(RDFUnitConfiguration configuration) throws ParameterException {
  ...
  private void setQueryTtlCachePaginationLimit(RDFUnitConfiguration configuration) throws ParameterException {
