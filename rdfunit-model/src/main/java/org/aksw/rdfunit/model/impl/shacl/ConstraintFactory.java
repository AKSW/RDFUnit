...
public final class ConstraintFactory {
  ...
  public static Set<ComponentConstraint> createFromShapeAndComponent(Shape shape,
      Component component) {
    ...
    getComponentShapeBindings(component, shape).forEach(bindingPairs -> {
      ...
      if (validators.isEmpty()) {
        log.warn("No validators found for {} Shape {} / component {} and bindings {}",
            shape.isPropertyShape() ? "Property" : "Node", shape.getElement(),
            component.getElement(), bindingPairs);
      }

      // for multiple validators
      validators.forEach(validator -> {
        ComponentConstraintImpl.ComponentConstraintImplBuilder constraintBuilder = ComponentConstraintImpl
            .builder();
  ...
  private static Set<Set<PropertySingleValuePair>> getComponentShapeBindings(Component component,
      Shape shape) {

    Set<Property> parameterProperties = component.getParameters().stream()
        .map(ComponentParameter::getPredicate).collect(Collectors.toSet());
    Set<PropertySingleValuePair> bindingPairs = shape.getPropertyValuePairSets().getAnnotations()
        .stream()
    ...
    Set<Property> optionalParametersWithDefaultValue = component.getParameters().stream()
        ...
        .map(ComponentParameter::getPredicate).collect(Collectors.toSet());
    Set<Property> existingPropertiesInBindings = bindingPairs.stream()
        .map(PropertySingleValuePair::getProperty).collect(Collectors.toSet());
    ...
    Set<Property> finalTotalPropertiesInBindings = bindingPairs.stream()
        .map(PropertySingleValuePair::getProperty).collect(Collectors.toSet());

    ImmutableSet.Builder<Set<PropertySingleValuePair>> pairs = ImmutableSet.builder();
    for (int i = Math.max(requiredParameterSize, finalTotalPropertiesInBindings.size());
        i <= maxParametersSize && i <= bindingPairs.size(); i++) {
    ...
    return pairs.build().stream()
        .filter(pairA -> {
          ...
          List<Property> pairAProperties = pairA.stream().map(PropertySingleValuePair::getProperty)
