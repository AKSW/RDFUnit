...
public class AggregatedTestCaseResultImpl extends StatusTestCaseResultImpl implements
   ...
   * @param prevalenceCount the prevalence count
   */
  public AggregatedTestCaseResultImpl(TestCase testCase, TestCaseResultStatus status,
      long errorCount, long prevalenceCount) {
    this(testCase.getElement(), testCase.getLogLevel(), testCase.getResultMessage(), status,
        errorCount, prevalenceCount);
  }

  public AggregatedTestCaseResultImpl(Resource testCaseUri, RLOGLevel severity, String message,
      TestCaseResultStatus status, long errorCount, long prevalenceCount) {
    super(testCaseUri, severity, message, status);
    this.errorCount = errorCount;
    this.prevalenceCount = prevalenceCount;
  }

  public AggregatedTestCaseResultImpl(Resource element, Resource testCaseUri, RLOGLevel severity,
      String message, XSDDateTime timestamp, TestCaseResultStatus status, long errorCount,
  ...
  public String toString() {
    ...
    return "Errors: " + errorCount + " / Prevalence: " + prevalenceCount + ". Test: "
        + getMessage();
  }
}
