...
public final class TestExecutionWriter implements ElementWriter {
  ...
  public static TestExecutionWriter create(TestExecution testExecution) {
    return new TestExecutionWriter(testExecution);
  }
  ...
  public Resource write(Model model) {
    ...
    resource.addProperty(PROV.startedAtTime,
        ...
        .addProperty(RDFUNITv.testsRun,
            ResourceFactory.createTypedLiteral(Long.toString(rs.getTotalTests()),
                XSDDatatype.XSDnonNegativeInteger))
        .addProperty(RDFUNITv.testsSuceedded,
            ResourceFactory.createTypedLiteral(Long.toString(rs.getSuccessfulTests()),
                XSDDatatype.XSDnonNegativeInteger))
        .addProperty(RDFUNITv.testsFailed,
            ResourceFactory.createTypedLiteral(Long.toString(rs.getFailedTests()),
                XSDDatatype.XSDnonNegativeInteger))
        .addProperty(RDFUNITv.testsTimeout,
            ResourceFactory.createTypedLiteral(Long.toString(rs.getTimeoutTests()),
                XSDDatatype.XSDnonNegativeInteger))
        .addProperty(RDFUNITv.testsError,
            ResourceFactory.createTypedLiteral(Long.toString(rs.getErrorTests()),
                XSDDatatype.XSDnonNegativeInteger))
        .addProperty(RDFUNITv.totalIndividualErrors,
            ResourceFactory.createTypedLiteral(Long.toString(rs.getIndividualErrors()),
                XSDDatatype.XSDnonNegativeInteger))
        .addProperty(SHACL.conforms,
            ResourceFactory.createTypedLiteral((rs.getIndividualErrors() == 0 ? "true" : "false"),
    ...
    for (TestCaseResult result : testExecution.getTestCaseResults()) {
      Resource resultIri = TestCaseResultWriter.create(result, testExecution.getTestExecutionUri())
          .write(model);
