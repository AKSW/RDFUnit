...
public class TestCaseGroupAnd implements TestCaseGroup {
  ...
  public TestCaseGroupAnd(@NonNull Set<? extends TargetBasedTestCase> testCases) {
    ...
    List<TestCaseAnnotation> innerAnnotations = testCases.stream()
        .map(GenericTestCase::getTestCaseAnnotation).collect(Collectors.toList());
    String description = "A logical AND constraint component containing the following rules:";
    if (innerAnnotations.size() == 1) {
      description = "";
    }
    description += innerAnnotations.stream().map(TestCaseAnnotation::getDescription)
        .collect(Collectors.joining(", "));
    this.testCaseAnnotation = new TestCaseAnnotation(
        this.resource,
        innerAnnotations.stream().map(TestCaseAnnotation::getGenerated).findFirst()
            .orElse(TestGenerationType.AutoGenerated),
        null,
        innerAnnotations.stream().map(TestCaseAnnotation::getAppliesTo).findFirst()
            .orElse(TestAppliesTo.Dataset),
        innerAnnotations.stream().map(TestCaseAnnotation::getSourceUri).findFirst()
            .orElse(SHACL.namespace),
        innerAnnotations.stream().flatMap(t -> t.getReferences().stream())
