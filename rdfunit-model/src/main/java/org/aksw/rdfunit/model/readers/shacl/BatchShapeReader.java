...
public final class BatchShapeReader {
  ...
  private static final ImmutableSet<Resource> shapesAsInstancesOf = ImmutableSet
      .of(SHACL.Shape, SHACL.NodeShape, SHACL.PropertyShape);
  private static final ImmutableSet<Property> shapesAsObjectsOf = ImmutableSet
      .of(SHACL.node, SHACL.property, SHACL.not);
  private static final ImmutableSet<Property> shapeListsAsObjectOf = ImmutableSet
      .of(SHACL.and, SHACL.or, SHACL.xone);
  private static final ImmutableSet<Property> shapesAsSubjectsOf = ImmutableSet
      .of(SHACL.node, SHACL.property, SHACL.and, SHACL.or, SHACL.xone, SHACL.not,
  ...
  public static BatchShapeReader create() {
    return new BatchShapeReader();
  }
  ...
  private void addShapesAsInstancesOf(Model model, ImmutableSet.Builder<Resource> shapes) {
    shapesAsInstancesOf
        .forEach(shapeType -> shapes.addAll(model.listSubjectsWithProperty(RDF.type, shapeType)));
  }
  ...
  private Collection<ShapeList> addShapeListsAsObjectsOf(Model model) {
    ...
    shapeListsAsObjectOf.forEach(property -> {
      ...
      Map<Resource, List<Tuple2<Resource>>> grouped = listShapes.stream()
          .collect(Collectors.groupingBy((Tuple2<Resource> t) -> t.get(0), Collectors.toList()));

      grouped.forEach((list, entries) -> {
        Shape listShape = ShapeReader.create().read(list);
        ImmutableSet<Shape> elements = ImmutableSet.copyOf(
            entries.stream().map(r -> ShapeReader.create().read(r.get(1)))
