...
public final class PatternReader implements ElementReader<Pattern> {
  ...
  public static PatternReader create() {
  ...
  public Pattern read(Resource resource) {
    ...
    for (Statement smt : resource.listProperties(DCTerms.identifier).toList()) {
      checkArgument(++count == 1, "Cannot have more than one identifier in Pattern %s",
          resource.getURI());
    ...
    for (Statement smt : resource.listProperties(DCTerms.description).toList()) {
      checkArgument(++count == 1, "Cannot have more than one description in Pattern %s",
          resource.getURI());
    ...
    for (Statement smt : resource.listProperties(RDFUNITv.sparqlWherePattern).toList()) {
      checkArgument(++count == 1, "Cannot have more than one SPARQL query in Pattern %s",
          resource.getURI());
    ...
    for (Statement smt : resource.listProperties(RDFUNITv.sparqlPrevalencePattern).toList()) {
      checkArgument(++count == 1, "Cannot have more than one prevalence query in Pattern %s",
          resource.getURI());
      patternBuilder.setSparqlPatternPrevalence(smt.getObject().asLiteral().getLexicalForm());
    }

    //parameters
    Collection<PatternParameter> patternParameters = resource.listProperties(RDFUNITv.parameter)
        .toList().stream()
    ...
    //annotations
    Collection<ResultAnnotation> patternAnnotations = resource
        .listProperties(RDFUNITv.resultAnnotation).toList().stream()
