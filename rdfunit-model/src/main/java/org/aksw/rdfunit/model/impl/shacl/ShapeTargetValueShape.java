...
public class ShapeTargetValueShape implements ShapeTarget {
  ...
  @Getter
  private final ShapeTargetType targetType = ShapeTargetType.ValueShapeTarget;
  ...
  private ShapeTargetValueShape(ShapeTarget innerTarget, List<ShapePath> pathChain,
      Function<ShapeTargetValueShape, String> generatePattern) {
    this.innerTarget = innerTarget;
    this.pathChain = ImmutableList
        .copyOf(pathChain.stream().filter(Objects::nonNull).collect(Collectors.toList()));
    this.generatePattern = generatePattern;
  }


  public static ShapeTarget create(ShapeTarget outerTarget, String property) {
    return create(outerTarget,
        ShapePathReader.create().read(ResourceFactory.createProperty(property)));
  }
  ...
  public static ShapeTarget create(ShapeTarget innerTarget, List<ShapePath> pathChain) {
    switch (innerTarget.getTargetType()) {
      case ClassTarget:
        return new ShapeTargetValueShape(innerTarget, pathChain,
            ShapeTargetValueShape::classTargetPattern);
      case NodeTarget:
        return new ShapeTargetValueShape(innerTarget, pathChain,
            ShapeTargetValueShape::nodeTargetPattern);
      case ObjectsOfTarget:
        return new ShapeTargetValueShape(innerTarget, pathChain,
            ShapeTargetValueShape::objectsOfTargetPattern);
      case SubjectsOfTarget:
        return new ShapeTargetValueShape(innerTarget, pathChain,
            ShapeTargetValueShape::subjectsOfTargetPattern);
  ...
  private static ShapeTarget transformTarget(ShapeTargetValueShape innerTarget,
      List<ShapePath> pathChain) {
  ...
  private static String classTargetPattern(ShapeTargetValueShape target) {
    return
        " ?focusNode <http://www.w3.org/1999/02/22-rdf-syntax-ns#type>/<http://www.w3.org/2000/01/rdf-schema#subClassOf>* "
            + formatNode(target.getNode()) + " ; \n" +
            writePropertyChain(target.pathChain) + "  ?this . \n";
  }

  private static String nodeTargetPattern(ShapeTargetValueShape target) {
    return " " + formatNode(target.getNode()) + " " + writePropertyChain(target.pathChain)
        + " ?this . \n" +
        "BIND (" + formatNode(target.getNode()) + " AS ?focusNode) .\n";
  }

  private static String objectsOfTargetPattern(ShapeTargetValueShape target) {
    return " ?focusNode (^" + formatNode(target.getNode()) + ")/" + writePropertyChain(
        target.pathChain) + " ?this . \n";
  }
