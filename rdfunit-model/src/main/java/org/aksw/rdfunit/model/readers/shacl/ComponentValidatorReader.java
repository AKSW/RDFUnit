...
public class ComponentValidatorReader implements ElementReader<ComponentValidator> {
  ...
  public static ComponentValidatorReader create(ComponentValidatorType type) {
    return new ComponentValidatorReader(type);
  }
  ...
  public ComponentValidator read(Resource resource) {
    ...
    ComponentValidatorImpl.ComponentValidatorImplBuilder validatorBuilder = ComponentValidatorImpl
        .builder();
    ...
    for (Statement smt : resource.listProperties(SHACL.ask).toList()) {
      checkArgument(type.equals(ComponentValidatorType.ASK_VALIDATOR),
          "SPARQL SELECT-Based Validator contains ASK query: %s",
          smt.getObject().asLiteral().getLexicalForm());
    ...
    for (Statement smt : resource.listProperties(SHACL.select).toList()) {
      checkArgument(!type.equals(ComponentValidatorType.ASK_VALIDATOR),
          "SPARQL ASK-Based Validator contains SELECT query %s",
          smt.getObject().asLiteral().getLexicalForm());
      validatorBuilder.sparqlQuery(smt.getObject().asLiteral().getLexicalForm());
    }
