...
public class ComponentConstraintImpl implements ComponentConstraint {
  ...
  private static final Pattern patternForExtractingFilterFromSimpleAskValidator = Pattern
      .compile("(?i)ASK\\s*\\{\\s*FILTER\\s*\\(([\\s\\S]*)\\)[\\s\\.]*\\}");
  private static final List<Property> nonValueArgs = ImmutableList
      .of(SHACL.minCount, SHACL.maxCount, SHACL.uniqueLang);
  @Getter
  @NonNull
  ...
  @NonNull
  @Singular
  private final ImmutableMap<ComponentParameter, RDFNode> bindings;
  @Getter(lazy = true)
  @NonNull
  ...
  public Literal getMessage() {
    Literal messageLiteral = shape.getMessage().orElseGet(
        () -> validator.getDefaultMessage().orElse(ResourceFactory.createStringLiteral("Error")));

    String messageLanguage = messageLiteral.getLanguage();
    String message = new MessagePrebinding(messageLiteral.getLexicalForm(), shape)
  ...
  private String generateSparqlWhereFromAskValidator(String sparqlString) {
    ...
    Matcher filterMatcher = patternForExtractingFilterFromSimpleAskValidator
        .matcher(sparqlString.trim());
