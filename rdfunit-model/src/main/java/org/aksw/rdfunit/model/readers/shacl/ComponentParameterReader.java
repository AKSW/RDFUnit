...
public final class ComponentParameterReader implements ElementReader<ComponentParameter> {
  ...
  public static ComponentParameterReader create() {
    return new ComponentParameterReader();
  }
  ...
  public ComponentParameter read(Resource resource) {
    ...
    ComponentParameterImpl.ComponentParameterImplBuilder argumentBuilder = ComponentParameterImpl
        .builder();
    ...
    for (Statement smt : resource.listProperties(SHACL.path).toList()) {
      argumentBuilder = argumentBuilder
          .predicate(ResourceFactory.createProperty(smt.getObject().asResource().getURI()));
    }
