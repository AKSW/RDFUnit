...
public final class BindingReader implements ElementReader<Binding> {
  ...
  public static BindingReader create(Pattern pattern) {
    return new BindingReader(pattern);
  }
  ...
  public Binding read(Resource resource) {
    ...
    for (Statement smt : resource.listProperties(RDFUNITv.bindingValue).toList()) {
      checkArgument(++count == 1, "Cannot have more than one rut:bindingValue in Binding %s",
          resource.getURI());
      value = smt.getObject();
    ...
    for (Statement smt : resource.listProperties(RDFUNITv.parameter).toList()) {
      checkArgument(++count == 1, "Cannot have more than one rut:parameter in Binding %s",
          resource.getURI());
