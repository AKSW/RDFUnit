...
public final class TestCaseResultReader implements ElementReader<TestCaseResult> {
  ...
  public static TestCaseResultReader create(Property messageProperty, Property severityProperty) {
    return new TestCaseResultReader(messageProperty, severityProperty);
  }
  ...
  public TestCaseResult read(final Resource resource) {
    ...
    for (Statement smt : resource.listProperties(DCTerms.date).toList()) {
      timestamp = (XSDDateTime) XSDDatatype.XSDdateTime
