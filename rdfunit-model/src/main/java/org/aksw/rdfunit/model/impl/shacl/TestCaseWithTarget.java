...
public class TestCaseWithTarget implements TestCase, TargetBasedTestCase {
  ...
  @Getter
  @NonNull
  private final Resource element = ResourceFactory.createProperty(JenaUtils.getUniqueIri());
  ...
  private static TestCaseAnnotation createFromReferences(TestCaseAnnotation annotation,
  ...
  public String getSparqlWhere() {
    ...
    if (target instanceof ShapeTargetValueShape) {
      ...
      withFocus = withFocus.replaceFirst("(?i)GROUP BY\\s+\\?" + CommonNames.This,
          "GROUP BY ?" + CommonNames.This + " ?focusNode");
    }
  ...
  public TestCaseAnnotation getTestCaseAnnotation() {
    ...
    return createFromReferences(testCase.getTestCaseAnnotation(),
        target.getRelatedOntologyResources());
  }
  ...
  public Set<ResultAnnotation> getVariableAnnotations() {
    ...
    if (ShapeTargetValueShape.class.isAssignableFrom(target.getClass())) {
      ...
      builder
          .add(new ResultAnnotationImpl.Builder(ResourceFactory.createResource(), SHACL.focusNode)
  ...
  private String injectSparqlSnippet(String sparqlQuery, String sparqlSnippet) {
    ...
    return sparqlQuery.substring(0, bracketIndex + 1) + sparqlSnippet + sparqlQuery
