...
public final class RdfResultsWriterFactory {
  ...
  public static RdfWriter createWriterFromFormat(String filenameWithoutExtension,
      SerializationFormat serializationFormat, TestExecution testExecution) {
    if (serializationFormat.equals(FormatService.getOutputFormat("html"))) {
      return createHtmlWriter(testExecution,
          filenameWithoutExtension + "." + serializationFormat.getExtension());
    } else if (serializationFormat.equals(FormatService.getOutputFormat("junitxml"))) {
      return createJunitXmlWriter(testExecution,
          filenameWithoutExtension + "." + serializationFormat.getExtension());
    } else {
      return new RdfFileWriter(filenameWithoutExtension + "." + serializationFormat.getExtension(),
          serializationFormat.getName());
    }
  }


  public static RdfWriter createWriterFromFormat(OutputStream outputStream,
      SerializationFormat serializationFormat, TestExecution testExecution) {
    if (serializationFormat.equals(FormatService.getOutputFormat("html"))) {
  ...
  public static AbstractRdfHtmlResultsWriter createHtmlWriter(TestExecution testExecution,
      String filename) {
  ...
  public static AbstractJunitXmlResultsWriter createJunitXmlWriter(TestExecution testExecution,
      String filename) {
    return createJunitXmlWriter(testExecution,
        RdfStreamWriter.getOutputStreamFromFilename(filename));
  }
