...
/**
 * Can create a 'trusting' SSL connection, accepting anything. Using apache HttpClient deals with
 * redirects automatically. Should only be used for crawling results of the LOV endpoint.
 */
final class TrustingUrlConnection {
  ...
  private final static Registry<ConnectionSocketFactory> registry;
  private static final HttpClientConnectionManager poolingConnManager;
  ...
  static HttpResponse executeHeadRequest(URI uri, SerializationFormat format) throws IOException {
    ...
    String acceptHeader =
        format.getMimeType() != null && !format.getMimeType().trim().isEmpty() ? format
            .getMimeType() : "*/*";
    CloseableHttpClient httpClient = HttpClientBuilder
        ...
        .setDefaultHeaders(Arrays.asList(
            ...
            //if default request we try to pretend to be a browser, else we accept everything
            new BasicHeader("User-Agent", "Mozilla/5.0"),
            new BasicHeader("Upgrade-Insecure-Requests", "1")
            // we are an all trusting client...
        ))
        .build();

    HttpResponse httpResponse = httpClient.execute(headMethod);
    redirectHandler.lastRedirectedUri
        .forEach(x -> httpResponse.setHeader(HEADERKEY, String.valueOf(x)));
    return httpResponse;
  }

  /**
   * Captures every redirect captured starting with the initial address. Is used to check whether we
   * are in a redirect cycle.
   */
