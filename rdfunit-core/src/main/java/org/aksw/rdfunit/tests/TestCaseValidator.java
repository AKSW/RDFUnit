...
public class TestCaseValidator {
  ...
  public void validate() {
    ...
    validateSPARQL(new QueryGenerationExtendedSelectFactory().getSparqlQueryAsString(testCase),
        "SPARQL Extended");
    validateSPARQL(new QueryGenerationCountFactory().getSparqlQueryAsString(testCase),
        "SPARQL Count");
    ...
    if (!testCase.getSparqlPrevalence().trim().isEmpty()) { // Prevalence in not always defined
      validateSPARQL(PrefixNSService.getSparqlPrefixDecl() + testCase.getSparqlPrevalence(),
          "prevalence");
    }

    Collection<String> vars = new QueryGenerationSelectFactory().getSparqlQuery(testCase)
        .getResultVars();
    ...
    if (testCase.getResultMessage().trim().isEmpty()) {
      ...
      log.warn("No test case dcterms:description message included in TestCase: {}",
          testCase.getTestURI());
    }
  ...
  private void validateSPARQL(String sparql, String type) {
    try {
      ...
      String message =
          "QueryParseException in " + type + " query (line " + e.getLine() + ", column " + e
              .getColumn() + " for Test: " + testCase.getTestURI() + "\n" + PrefixNSService
              .getSparqlPrefixDecl() + sparql;
