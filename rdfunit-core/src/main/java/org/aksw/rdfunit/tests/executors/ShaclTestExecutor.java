...
/**
 * The Extended Test Executor extends RLOG Executor but provides richer error metadata TODO: At the
 * moment this is partially
 *
  ...
  protected Collection<TestCaseResult> executeSingleTest(TestSource testSource, TestCase testCase)
    ...
    try (QueryExecution qe = testSource.getExecutionFactory()
      ...
      while (results.hasNext()) {
        ...
        ShaclTestCaseResultImpl.Builder resultBuilder = new ShaclTestCaseResultImpl.Builder(
            testCase.getElement(), logLevel, message, focusNode);
        ...
        for (ResultAnnotation resultAnnotation : testCase.getResultAnnotations()) {
          ...
          if (resultAnnotation.getAnnotationValue().isPresent()) {
            ...
            annotationSetBuilder.annotation(
                PropertyValuePair.create(resultAnnotation.getAnnotationProperty(),
        ...
        for (ResultAnnotation resultAnnotation : testCase.getVariableAnnotations()) {
          ...
          if (resultAnnotation.getAnnotationVarName().isPresent()) {
            ...
            if (qs.contains(variable)) {
              annotationSetBuilder.annotation(
                  PropertyValuePair
                      .create(resultAnnotation.getAnnotationProperty(), qs.get(variable)));
            }
          }
        }
