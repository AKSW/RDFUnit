...
import org.aksw.rdfunit.tests.query_generation.QueryGenerationFactoryCache;
...
public final class TestExecutorFactory {
  ...
  /**
   * Creates a test executor based on a TestCaseExecutionType. The executor has an internal cache
   * that caches the parsed SPARQL query object this speeds up test execution when the same test
   * suite is called repeatedly on different datasets
   */
  public static TestExecutor createTestExecutorWithCache(TestCaseExecutionType executionType,
      int cacheSize) {
    QueryGenerationFactory qgf = createQueryGeneration(executionType);
