...
public class SimpleTestExecutorMonitor implements TestExecutorMonitor {
  ...
  @Override
  public void singleTestExecuted(GenericTestCase test, TestCaseResultStatus status,
    ...
    if (results.size() == 1) {
      ...
      if (result instanceof StatusTestCaseResult) {
        ...
        if (loggingEnabled) {
          log.info(
              "Test " + counter + "/" + overviewResults.getTotalTests() + " returned " + result);
        }
    ...
    if (!statusResult) {
      ...
      if (loggingEnabled) {
        log.info("Test " + counter + "/" + overviewResults.getTotalTests() + " returned " + results
            .size() + " violation instances / TC: " + test.getAbrTestURI());
      }
