...
/**
 * The Simple SHACL Executor returns violation instances and for every instance it generates an
 * sd:Violation:Entry See the rlog vocabulary
 *
  ...
  protected Collection<TestCaseResult> executeSingleTest(TestSource testSource, TestCase testCase)
    ...
    try (QueryExecution qe = testSource.getExecutionFactory()
      ...
      qe.execSelect().forEachRemaining(qs -> {
        ...
        testCaseResults.add(
            new ShaclLiteTestCaseResultImpl(testCase.getElement(), logLevel, message, focusNode));
      });
