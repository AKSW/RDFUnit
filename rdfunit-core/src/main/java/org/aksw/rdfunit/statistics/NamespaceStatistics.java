...
public final class NamespaceStatistics {
    ...
    private NamespaceStatistics(Collection<DatasetStatistics> datasetStatisticses, boolean skipUnknownNamespaces, RDFUnitConfiguration conf) {
        ...
        if(conf != null) {
            this.excludeSchemata = conf.getExcludeSchemata().stream().map(SchemaSource::getSourceConfig).collect(Collectors.toList());
        }
        else{
