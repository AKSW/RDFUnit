...
/**
 * Defines a source based on an RDF Dump This can be any type of dump (ttl, nt, rdfa, rdf, etc) This
 * has to be more sophisticated in the end (e.g. read zippped files) but there is always time to
 * improve ;) TODO refactor sources and make this a subclass of DatasetSource
 *
...
public class DatasetTestSource extends AbstractTestSource implements TestSource {
  ...
  DatasetTestSource(SourceConfig sourceConfig, QueryingConfig queryingConfig,
      Collection<SchemaSource> referenceSchemata, RdfReader dumpReader, Dataset dataset) {
  ...
  DatasetTestSource(SourceConfig sourceConfig, QueryingConfig queryingConfig,
      Collection<SchemaSource> referenceSchemata, RdfReader dumpReader) {
    this(sourceConfig, queryingConfig, referenceSchemata, dumpReader,
        DatasetFactory.create());  //OntModelSpec.RDFS_MEM_RDFS_INF
  }

  DatasetTestSource(DatasetTestSource datasetTestSource,
      Collection<SchemaSource> referenceSchemata) {
    this(datasetTestSource.sourceConfig, datasetTestSource.queryingConfig, referenceSchemata,
        datasetTestSource.dumpReader, datasetTestSource.dumpDataset);
  }
  ...
  protected QueryExecutionFactory initQueryFactory() {
    ...
    OntModel ontModel = ModelFactory
        .createOntologyModel(OntModelSpec.RDFS_MEM_RDFS_INF, ModelFactory.createDefaultModel());
    try {
      ...
      throw new IllegalArgumentException(
          "Cannot read dump URI: " + getUri() + " Reason: " + e.getMessage(), e);
    }
