...
public class RDFUnitStaticValidatorTest {
  ...
  public void testValidateModelAsSource() throws RdfReaderException {
    RDFUnitStaticValidator.validate(testCaseExecutionType, ModelFactory.createDefaultModel(),
    ...
    final TestSource modelSource = new TestSourceBuilder()
        ...
        .setInMemReader(new RdfModelReader(
            RdfReaderFactory.createResourceReader("/org/aksw/rdfunit/validate/data/empty.ttl")
                .read()))
        .setReferenceSchemata(Collections.emptyList())
        .build();
    RDFUnitStaticValidator
        .validate(testCaseExecutionType, modelSource, RDFUnitStaticValidator.getTestSuite());
  }
