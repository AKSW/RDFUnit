...
final class SchemaSelectorView extends CustomComponent implements WorkflowItem {
  ...
  private void setDefaultValues() {
    messageLabel.setValue(
        "Note that we do not provide full OWL/DSP/RS (but will in time) support see our <a href='https://github.com/AKSW/RDFUnit/wiki' target=\"_blank\">wiki</a> "
            +
  ...
  private void setInputText() {
    ...
    ontologyMessage.setValue(
        "<h3>Automatic Constraint Detection for Ontologies (including OWL, RDFS, RDF, XSD)</h3>" +
  ...
  public boolean execute() {
    ...
    switch (schemaOption) {
      ...
      case SPECIFIC_URIS:
        ...
        break;
      case AUTO_ONTOLOGY:
        configuration.setExcludeSchemataFromPrefixes(
            Lists.newArrayList());     //remove default excluded ontologies
        configuration.setAutoSchemataFromQEF(configuration.getTestSource().getExecutionFactory());
        break;
      case CUSTOM_TEXT:
        ...
        try {
          ...
          SchemaSource source = SchemaSourceFactory
              .createSchemaSourceFromText("http://rdfunit.aksw.org/CustomConstraint#", text,
                  oficialFormat);
    ...
    this.setMessage(
        "Constraints loaded successfully: (" + getSchemaDesc(configuration.getAllSchemata()) + ")",
        false);
    return true;
  }
