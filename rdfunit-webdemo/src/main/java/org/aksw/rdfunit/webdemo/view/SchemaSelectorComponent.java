...
/**
 * @author Dimitris Kontokostas provides a schema
...
final class SchemaSelectorComponent extends VerticalLayout {
  ...
  /**
   * This is the window used to add new contacts to the 'address book'. It does not do proper
   * validation - you can add weird stuff.
   */
          ...
          new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
              ...
              if (!(prefix == null || uri == null || prefix.isEmpty() || uri.isEmpty())) {
                SchemaSource source = SchemaSourceFactory.createSchemaSourceSimple(prefix, uri,
                    RdfReaderFactory.createDereferenceReader(uri));
                ((BeanItemContainer) f.getContainerDataSource())
                    .addBean(source);
                f.addToken(source);
              } else {
                Notification
                    .show("Invalid source for prefix: \"" + prefix + "\" and URI: " + uri + "\"");
              }
              close();
  ...
  public static class RemoveWindow extends Window {
    ...
    RemoveWindow(final Source s, final TokenField f) {
      ...
      l.addComponent(
          new Label("Are you sure you want to remove " + s.getPrefix() + "?\n" + s.getUri()));
