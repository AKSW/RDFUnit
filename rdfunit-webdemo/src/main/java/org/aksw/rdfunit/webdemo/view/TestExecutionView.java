...
//    }
  ...
  private void initLayout() {
    ...
    startTestingButton
        .addClickListener((Button.ClickListener) event -> UI.getCurrent().access(() -> {
    ...
    startTestingCancelButton
        .addClickListener((Button.ClickListener) clickEvent -> UI.getCurrent().access(() -> {
          if (!inProgress) {
            Notification.show("Test Execution Not in progress", Notification.Type.WARNING_MESSAGE);
            return;
          }
          TestExecutionView.this
              .setMessage("Sending cancel signal, waiting for current test to execute", false);
          CommonAccessUtils.pushToClient();
    ...
    resultsButton
        .addClickListener((Button.ClickListener) clickEvent -> UI.getCurrent().access(() -> {
          if (!isReady || inProgress) {
            Notification.show("Test Execution in progress or not completed",
          ...
          String resultFormat;
          try {
            resultFormat = FormatService.getOutputFormat(resultsFormatsSelect.getValue().toString())
                .getName();
          } catch (NullPointerException e) {
            Notification
                .show("Error occurred in format selection", Notification.Type.ERROR_MESSAGE);
          ...
          try {
            ...
            String fileLocation =
                RDFUnitDemoSession.getBaseDir() + "results/" + System.currentTimeMillis() + "-"
                    + randomNumber + ".ttl";
          ...
          if (resultFormat.equals("html")) {
            BrowserFrame frame = new BrowserFrame("", new ConnectorResource() {
              ...
              public DownloadStream getStream() {
                try {
                  return new DownloadStream(new ByteArrayInputStream(contents.getBytes("UTF8")),
                      "text/html", "");
                } catch (UnsupportedEncodingException e) {
                  LOGGER.error("Encoding error", e);
                }
  ...
  private TestExecutorMonitor createProgressMonitor() {
    return new TestExecutorMonitor() {
      ...
      @Override
      public void singleTestExecuted(final GenericTestCase test, final TestCaseResultStatus status,
          final java.util.Collection<TestCaseResult> results) {
        long errors = 0;
...
//                }
      ...
      public void testingFinished() {
        UI.getCurrent().access(() -> {
          ...
          setMessage("Completed! " + getStatusStr()
              + ". See the results or rerun with a different 'Report Type'", false);
      ...
      private String getStatusStr() {
        return " (S: " + sucessTests + " / F: " + failTest + " / E: " + timeoutTests + " / T : "
            + totalErrors + ")";
      }
    };
