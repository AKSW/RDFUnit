...
public final class DqvReportWriter implements ElementWriter {
  ...
  public static DqvReportWriter create(Collection<QualityMeasure> measures) {
    return new DqvReportWriter(measures);
  }
  ...
  public Resource write(Model model) {
    ...
    for (QualityMeasure m : measures) {
      ...
      Resource measureResource = model.createResource(JenaUtils.getUniqueIri())
          ...
          .addProperty(ResourceFactory.createProperty(DQV_NS + "hasMetric"),
              ResourceFactory.createResource(m.getDqvMetricUri()))
          .addProperty(ResourceFactory.createProperty(DQV_NS + "value"),
              Double.toString(m.getValue()));

      te.addProperty(ResourceFactory.createProperty(DQV_NS + "dqv:hasQualityMeasure"),
          measureResource);
